...
output$plot_keto <- renderPlot({
  ggplot(d[Date > "2017-10-01"], aes(x=Date, y=Weight)) + 
    ...
    geom_smooth(data=d[Date <= "2018-07-31"][Date > "2017-10-01"], color=6, fill=6, alpha=.15, size=.5, method="loess") + 
...
output$plot_since_14 <- renderPlot({
  ggplot(d, aes(x=Date, y=Weight)) + 
    ...
    geom_smooth(data=d[Date >= "2017-01-01"][Date <= "2018-07-31"], color=6, fill=6, alpha=.15, size=.5, method="loess", span=.35) + 
