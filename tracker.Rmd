...
## join race dates
races <- data.table(Race=c(##"Garmin", "Kansas City", "Buckeye", "Free State", "Oregon", "St. George", 
                          "Psycho Wyco", "Free State", "Alamo City", "Cactus Rose", "Bandera 22", 
                          "Salt Lake", "Sky Island", "Austin Rattler", "Bandera 23", "Elephant Rock", 
                          "Wasatch", "Bandera 24"), 
                    Date=as.Date(c(##"2018-04-21", "2018-10-20", "2018-12-08", "2019-04-27", "2019-07-20", "2019-10-05", 
                                   "2020-02-22", "2021-06-12", "2021-09-11", "2021-10-22", "2022-01-08", 
                                   "2022-04-23", "2022-09-17", "2022-11-05", "2023-01-07", "2023-08-12",  
...
output$wt_plot <- renderPlot({
  ggplot(d[Date > "2020-01-01"], aes(x=Date, y=Rolling, label=Race)) + ## Previously: 2017-10-01
    # plot and smooth weight
    geom_line(aes(x=Date, y=Int), color="dodgerblue", size=1, alpha=.7) + 
    geom_smooth(data=d[Date >= "2020-01-01"], color="deeppink", fill="deeppink", alpha=.3, size=.5, method="loess", span=0.15) + 
