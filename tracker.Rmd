...
output$plot_keto <- renderPlot({
  ggplot(d[Date > "2017-10-01"], aes(x=Date, y=Weight)) + 
    ...
    geom_smooth(data=d[Date %between% c("2017-10-01", "2018-07-31")], color=6, fill=6, alpha=.15, size=.5, method="loess") + 
    geom_smooth(data=d[Date %between% c("2018-09-03", "2019-03-03")], color=6, fill=6, alpha=.15, size=.5, method="loess") + 
    #geom_smooth(data=d[Date %between% c("2018-11-30", "2019-01-03")], color=6, fill=6, alpha=.15, size=.5, method="loess") + 
    geom_smooth(data=d[Date >= "2019-03-08"], color=6, fill=6, alpha=.15, size=.5, method="loess") + 
    labs(x="", y="Weight (Lbs)") + 
    theme_minimal()
})
...
output$plot_since_14 <- renderPlot({
  ggplot(d, aes(x=Date, y=Weight)) + 
    ...
    geom_smooth(data=d[Date %between% c("2017-01-01", "2018-07-31")], color=6, fill=6, alpha=.15, size=.5, method="loess", span=.5) + 
    geom_smooth(data=d[Date %between% c("2018-09-03", "2019-03-03")], color=6, fill=6, alpha=.15, size=.5, method="loess", span=.5) + 
    geom_smooth(data=d[Date >= "2019-03-08"], color=6, fill=6, alpha=.15, size=.5, method="loess", span=.25) + 
    labs(x="", y="Weight (Lbs)") + 
    theme_minimal()
})
